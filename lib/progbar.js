...
ProgressBar(options)
{
	...
	self.pb_rlif = mod_readline.createInterface(fakeStdin, self.pb_tty);
	self.pb_rlif.setPrompt('');
...
end()
{
	...
	if (self.pb_hide_cursor && DEC_TERMINAL)
		self.pb_rlif.write(ECMA48.CSI + DEC.TCEM + DEC.SET);
...
draw()
{
	...
	if (self.pb_hide_cursor && DEC_TERMINAL && self.pb_lastdrawtime === 0)
		self.pb_rlif.write(ECMA48.CSI + DEC.TCEM + DEC.RST);
	...
	if (ECMA48_TERMINAL) {
		 ...
		 * flicker than erasing the entire line before printing
		 * again.
		 */
		self.pb_rlif.write(ECMA48.CSI + '1' + ECMA48.CHA);
	} else {
		/*
		 * Otherwise, assume readline knows how to clear the line
		 * before we repaint it.
		 */
		self.pb_rlif.write(null, { ctrl: true, name: 'u' });
	}
	self.pb_rlif.write(filestr + bar + infostr);
